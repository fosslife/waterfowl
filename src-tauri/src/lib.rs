...
use sqlx::{Column, TypeInfo, ValueRef, postgres::PgRow};
...
fn decode_pg_value(row: &PgRow, ordinal: usize, type_name: &str) -> Value {
    match type_name {
        ...
        // ===== Numeric/Decimal =====
        "NUMERIC" | "DECIMAL" => {
            match row.try_get::<sqlx::types::BigDecimal, _>(ordinal) {
        ...
        "BYTEA" => {
            ...
            if bytes.len() > 100 {
                Value::String(format!("\\x{}... ({} bytes)", hex::encode(&bytes[..50]), bytes.len()))
            } else {
        ...
        "TIMETZ" => {
            ...
            match row.try_get::<sqlx::postgres::types::PgTimeTz, _>(ordinal) {
                Ok(t) => Value::String(format!("{}{:+}", t.time, t.offset.local_minus_utc() / 3600)),
        ...
        "INTERVAL" => {
            match row.try_get::<sqlx::postgres::types::PgInterval, _>(ordinal) {
                Ok(interval) => {
        ...
        // ===== Network types =====
        "INET" => {
            match row.try_get::<ipnetwork::IpNetwork, _>(ordinal) {
        ...
        // ===== Geometric types =====
        "POINT" => {
            match row.try_get::<sqlx::postgres::types::PgPoint, _>(ordinal) {
        ...
        "LSEG" => {
            decode_pg_raw_text(row, ordinal, "LSEG")
        },
        "BOX" => {
            match row.try_get::<sqlx::postgres::types::PgBox, _>(ordinal) {
        ...
        "PATH" => {
            decode_pg_raw_text(row, ordinal, "PATH")
        },
        "POLYGON" => {
            decode_pg_raw_text(row, ordinal, "POLYGON")
        },
        "CIRCLE" => {
            decode_pg_raw_text(row, ordinal, "CIRCLE")
        },
        
        // ===== Range types =====
        "INT4RANGE" => {
            match row.try_get::<sqlx::postgres::types::PgRange<i32>, _>(ordinal) {
                Ok(r) => format_pg_range(&r),
                Err(_) => Value::String("[INT4RANGE]".to_string())
            }
        },
        "INT8RANGE" => {
            match row.try_get::<sqlx::postgres::types::PgRange<i64>, _>(ordinal) {
                Ok(r) => format_pg_range(&r),
        ...
        "TSTZRANGE" => {
            match row.try_get::<sqlx::postgres::types::PgRange<chrono::DateTime<chrono::Utc>>, _>(ordinal) {
                Ok(r) => format_pg_range(&r),
        ...
        // ===== Text search types =====
        "TSVECTOR" => {
            decode_pg_raw_text(row, ordinal, "TSVECTOR")
        },
        "TSQUERY" => {
            decode_pg_raw_text(row, ordinal, "TSQUERY")
        },
        ...
        "_INET" => {
            match row.try_get::<Vec<ipnetwork::IpNetwork>, _>(ordinal) {
                Ok(arr) => {
...
fn format_pg_range_decimal(range: &sqlx::postgres::types::PgRange<sqlx::types::BigDecimal>) -> Value {
    use std::ops::Bound;
